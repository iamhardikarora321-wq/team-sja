...
const getScrollDistance = () => section ? clamp(-section.getBoundingClientRect().top, 0, section.offsetHeight - window.innerHeight) : 0;
...
// Infinite Slider Logic with Keyboard Navigation
function setupSightSlider() {
  if (!track) return;
  if (track.replaceChildren) {
...
function updateSightSlider() {
  ...
  sightCards.forEach(card => card.classList.remove("is-active"));
  if (sightCards[activeSight]) sightCards[activeSight].classList.add("is-active");
}
...
// Keyboard global arrows for slider
window.addEventListener("keydown", e => {
  if (sightsControls && sightsControls.classList.contains("is-ready")) {
    if (e.key === "ArrowLeft") moveSightSlider(-1);
    if (e.key === "ArrowRight") moveSightSlider(1);
  }
});
